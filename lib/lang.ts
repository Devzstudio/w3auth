const Lang = {
    INVALID_REFRESH_TOKEN: "Invalid refresh token.",
    INVALID_ADDRESS: "Invalid address",
    USER_NOT_FOUND: "User not found",
    NOT_ON_ALLOWED_LIST: "Wallet address not found on allowlist",
    ADDRESS_BLOCKED: "Your wallet address is blocked",
    NFT_NOT_FOUND: "You dont have the NFT to access site",
    INVALID_SIGNATURE: "Invalid signature",
    INCORRECT_WALLET_ADDRESS: "Your wallet address and signed message wallet address is not same.",
    EXPIRED_NONCE: "Expired sign-in. Try again",
    INCORRECT_CRON_PASSWORD: "Incorrect cron password"
}

export default Lang;
